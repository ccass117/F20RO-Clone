...
class SupervisorGA:
    def __init__(self):
        ...
        if self.light_node is None:
            sys.stderr.write("No DEF Spolight node found in the current world file\n")
            sys.exit(1)
        self.light_intensity_field = self.light_node.getField("intensity")      
    ...
    def evaluate_genotype(self,genotype,generation):
        ...
        while currentInteraction < numberofInteractionLoops:
            ...
            self.emitterData = str(genotype)
             ...
             # Randomly toggle light on or off
            light_state = random.choice([0.0, 3.0])  # Randomly set to 0 (off) or 3 (on)
            self.light_intensity_field.setSFFloat(light_state)
            print(f"light intensity = " + str(light_state))            
