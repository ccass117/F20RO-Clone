...
class Controller:
    ...
    def calculate_fitness(self):
        # Retrieve the velocity of both motors
        left_motor_actuation = self.left_motor.getVelocity()
        right_motor_actuation = self.right_motor.getVelocity()
        
        # 1. Speed term: average speed of both motors
        V = (left_motor_actuation + right_motor_actuation) / 2
        
        # 2. Direction difference term: absolute difference between the speeds
        delta_v = abs(left_motor_actuation - right_motor_actuation)
        
        # 3. Obstacle avoidance term: use the proximity sensor with the highest activation value
        proximity_readings = [sensor.getValue() for sensor in self.proximity_sensors]
        i = max(proximity_readings) / 4095  # Normalize the sensor readings to [0, 1]
        
        # 4. Object avoidance: Dynamic penalty based on proximity readings
        object_avoidance_penalty = i * 2  # Scales penalty based on proximity
        
        # 5. Spinning avoidance: Apply a stricter penalty based on wheel speed difference
        spinning_penalty = delta_v * 1.25 if delta_v > 0.5 else 0  # More sensitive to speed difference
    
        # Fitness function: Φ = V * (1 - sqrt(Δv)) * (1 - i) - penalties
        forward_fitness = V * (1 - math.sqrt(delta_v)) * (1 - i)
        
        # Combine the penalties into the total fitness calculation
        total_fitness = forward_fitness - object_avoidance_penalty - spinning_penalty
        
        # Store and calculate the final fitness value
        self.fitness_values.append(total_fitness)
