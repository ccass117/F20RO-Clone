...
class Controller:
    def __init__(self, robot):        
        ...
        self.distance_to_reward_zone = float('inf')
    ...
    def calculate_fitness(self):
        # Forward Movement Fitness
        forward_fitness = abs(self.velocity_left) + abs(self.velocity_right)

        # Collision Avoidance Fitness
        collision_penalty = sum([sensor.getValue() for sensor in self.proximity_sensors]) / len(self.proximity_sensors)
        normalized_collision_penalty = (collision_penalty / 4095)
        avoid_collision_fitness = 1 - normalized_collision_penalty  # Normalize to range [0,1]
        if(avoid_collision_fitness < 0.9):
            avoid_collision_fitness = 0
        # Spinning Fitness
        spin_penalty = abs(self.velocity_left - self.velocity_right)
        spinning_fitness = 1 - (spin_penalty / self.max_speed)  # Normalize to range [0,1]
    
        # Proximity Fitness (Exponential Decay)
        k = 1.0  # Adjust the scaling factor as needed
        proximity_fitness = math.exp(-k * self.distance_to_reward_zone)
        close_proxy = 100000000 if (self.distance_to_reward_zone < 0.05) else 0
        
        if (close_proxy == 100000000):
            print("Im in the good boy zone")
   
        
        # Combined Fitness
        combined_fitness = (forward_fitness * avoid_collision_fitness * spinning_fitness + proximity_fitness) + close_proxy
    ...
    def handle_receiver(self):
        if self.receiver.getQueueLength() > 0:
            while(self.receiver.getQueueLength() > 0):
                if (len(self.receiver.getString())) < 30:
                    self.distance_to_reward_zone = float(self.receiver.getString())
    ...
    def check_for_spotlight(self):
        # Define a threshold to detect the spotlight (you may need to adjust this based on your environment)
        threshold = 100  # Example threshold value, adjust as necessary

        
        # Check if any light sensor detects a value above the threshold
        for sensor in self.light_sensors:
            sensor_value = sensor.getValue()
            if sensor_value < threshold:
                self.emitter.send("light_on")
                return True # Exit after detecting the light, or remove return to check all sensors
